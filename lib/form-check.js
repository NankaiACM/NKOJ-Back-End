...
const default_rules = {
  ...
  perm:['bool bit25',{func: val=>{return /^[0-1]{25,25}$/.test(val)}}],
  negative:['bool',{min:0,max:1}]
}
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (value === undefined) {
      if (rule.nonexist !== 'allow' && rule.empty !== 'remove') {
        ret.push({name: key, error: 'is undefined'})
        continue
      }
    }
    ...
    if (value === '') {
      switch (rule.empty) {
        ...
        case undefined:
        case 'error':
          ret.push({name: key, error: 'is empty'})
        // fall through
        case 'allow':
          continue
      }
    ...
    if (default_rules[type] !== undefined) {
      if (!check_internel(value, default_rules[type][1]))
        ret.push({name: key, error: default_rules[type][0]})
    }
    values[i] = value
