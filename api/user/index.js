...
const queryUserInfo = async function(uid) {
  ...
  const ac = await db.query('with t as (' +
      ...
      ') select ARRAY((select distinct problem_id from t where status_id = 107)) as ac, ARRAY((select distinct problem_id from t)) as all',
      [uid]);
...
router.get('/info/:nickname', require_perm(), fc.all(['nickname']),
    async (req, res) => {
      'use strict';
      const user = req.fcResult.nickname;
      console.log(user);
      try {
        const user_id_result = await db.query(
