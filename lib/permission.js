...
const require_perm = (permission) => {
  return async (req, res, next) => {
    ...
    // DEV:
    if(req.query.dev) return next();
    if (!permission) return next();
    if (!req.session.perm) {
      const result = await db.query(
          'SELECT cal_perm($1) AS perm', [req.session.user]
      );
      if (result.rows.length) req.session.perm = result.rows[0].perm;
    }
    req.session.perm && (req.session.perm[permission] === '1')
        ? next()
        : res.fatal(403);
  };
};

const check_perm = async (req, permission) => {
  if (!req.session.user) return false;
  if (!permission) return true;
  if (!req.session.perm) {
    const res = await db.query('SELECT cal_perm($1) AS perm',
        [req.session.user]);
    if (res.rows.length) {
      req.session.perm = res.rows[0].perm;
      return req.session.perm[permission] === '1';
    }
    return false;
  }
  return req.session.perm[permission] === '1';
