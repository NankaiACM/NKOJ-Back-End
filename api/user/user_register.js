...
router.get('/verify/:email', captcha.check('sendmail'), fc.all('email'),
    require_limit('sendmail'), async (req, res) => {
      'use strict';
      ...
      const link = `${require(
          '../../config/basic').BASE_URL}/api/u/verify/${key}/${code}`;
...
router.get('/verify/:key/:email', require_limit('sendmail'),
    async (req, res) => {
      'use strict';
      const key = req.params.key;
...
router.post('/register',
    fc.all(['nickname', 'password', 'email', 'school', 'gender']),
    async (req, res) => {
      'use strict';
