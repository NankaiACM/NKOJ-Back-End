...
const rules = {
  ...
  user: [
    oneOf(
        [getCheckSchema('user', 'email'), getCheckSchema('user', 'nickname')])],
    ...
    for (let rule of r) {
      try {
        await new Promise(async (resolve, reject) => {
          ...
          let handler = rules[rule] ||
              getCheckSchema(key, default_rules[key] ? rule : 'none');
          const handlerCallback = () => {
            const errors = validationResult(req).formatWith(formatter);
            if (!errors.isEmpty()) {
              res.fail ? res.fail(422, errors.array()) : res.status(422).
                  json(errors.array());
              reject();
            }
            req.fcResult = req.fcResult
                ? {...req.fcResult, ...matchedData(req)}
                : {...matchedData(req)};
            resolve();
          };
          if (!Array.isArray(handler)) handler = [handler];
          await new Promise(
              () => {handler.forEach((v) => { v(req, res, handlerCallback);});});
          resolve();
        });
      } catch (e) {
