...
const default_rules = {
  ...
  nickname: check('nickname').not().isEmail().toInt().not().isInt().withMessage('could not be pure number or something like an email address').isLength({
    min: 3,
    max: 20
  }).withMessage('3 < length < 20'),
  ...
  user: check('user').exists().withMessage('not exist').customSanitizer((val) => {
    val = val.toLowerCase()
    return val
  }),
