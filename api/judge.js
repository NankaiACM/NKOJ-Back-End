...
router.post('/', require_perm(), fc.all(['pid', 'lang', 'code']), async (req, res, next) => {
  ...
  socket.on('close', async function close () {
    try {
      ...
      let detail
      try {
        detail = fs.readFileSync(struct.file.detail, 'utf8').split('\n')
      } catch (e) {
        detail = 'not found'
      }
      let ac_count = 0
      console.log("datail:" + detail)
      detail.forEach(function(ecase, index){
        const case_res = ecase.split(':')[1]
        if(typeof(case_res) != "undefined"){
          console.log("case_res:" + case_res)
          if(case_res.indexOf("107") >= 0 || case_res.indexOf("108") >= 0){
            ac_count += 1
          }
        }
      })
...
router.post('/new_judge', require_perm(), fc.all(['pid', 'lang', 'code']), async (req, res, next) => {
  'use strict'
  const form = req.fcResult
  const pid = form.pid
  const lang = form.lang
  ...
  let cmd_str = "docker exec -i judgecore /usr/bin/judgecore/judgecore "
  let core_json = {}
  core_json.sid = solution_id
  core_json.filename = `/main.${langString}`
  core_json.lang = lang
  core_json.pid = pid
  core_json.max_time = time_limit
  core_json.max_time_total = time_limit*cases // TODO: add min(CONST_TIME, max_time_total)
  core_json.max_memory = memory_limit
  core_json.max_output = 10000000
  core_json.on_error_continue = ["accepted", "presentation error", "wrong answer"]
  core_json.test_case_count = cases
  core_json.spj_mode = special_judge
  core_json.spj_exec = null
  core_json.base_dir = "/mnt/data"
  core_json.debug = true
  core_json.pid = pid
